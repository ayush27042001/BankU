<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/NeoxAdmin.Master" AutoEventWireup="true" CodeBehind="ManageApiReq.aspx.cs" Inherits="NeoXPayout.Admin.ManageApiReq" %>
    ...
    <asp:GridView ID="gvRequests" runat="server" AutoGenerateColumns="False" DataKeyNames="Id"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="API Use">
                <ItemTemplate>
                    <span class="truncate-text" title='<%# Eval("APIUsecase") %>'>
                        <%# Eval("APIUsecase") %>
                    </span>
                    <button type="button" class="btn btn-link p-0 ms-2 text-primary copy-btn" 
                        data-text='<%# Eval("APIUsecase") %>' title="Copy">
            ...
            <asp:TemplateField HeaderText="Website URL">
                <ItemTemplate>
                    <span class="truncate-text" title='<%# Eval("WebsiteUrl") %>'>
                        <%# Eval("WebsiteUrl") %>
                    </span>
                    <button type="button" class="btn btn-link p-0 ms-2 text-primary copy-btn" 
                        data-text='<%# Eval("WebsiteUrl") %>' title="Copy">
                        <i class="bi bi-clipboard"></i>
                    </button>
                </ItemTemplate>
            </asp:TemplateField>
                ...
                <ItemTemplate>
                    <div class="d-flex gap-2">
                        ...
                        <asp:Button ID="btnUpdate" runat="server" Text="Update" CommandName="UpdateStatus"
                             CommandArgument='<%# Eval("Id") + "|" + Eval("UserId") + "|" + Eval("ReqId") +"|" + Eval("Title") %>' CssClass="btn btn-sm px-3" 
